...
static thread_local Fiber* t_fiber = nullptr;   //在运行的协程
...
Fiber::Fiber()
{
    ...
    if (getcontext(&m_ctx)) //保存当前上下文,初始化m_ctx
    {
...
m_cb(cb), m_runInScheduler(run_in_scheduler)
{
    ...
    makecontext(&m_ctx, &Fiber::MainFunc, 0);   //指定跳转函数
...
void Fiber::resume()
{
    ...
    {
        SetThis(this);
        if (swapcontext(&(t_thread_fiber->m_ctx), &m_ctx))  //切换到本上下文中
        {
...
void Fiber::yield()
{
    ...
    if (m_runInScheduler)
    {
        ...
        if (swapcontext(&m_ctx, &(t_thread_fiber->m_ctx)))  //切换回主线程上下文中
        {
...
void Fiber::MainFunc()
{
    ...
    cur.reset();    //注意不是Fiber的reset(),这里释放cur
    raw_ptr->yield();
}
